               ...
               <div class="animated-panel zoomIn element-item transition <?=$q['conteudo'][0]['CD_CONTEUDO']?> " data-category="transition" style="width:<?=(($q['tipo'] == 'O')? '25' : '49')?>%">
                    <div class="hpanel <?=(($q['tipo'] == 'O')? 'hgreen' : 'hyellow')?> ">
                        <div class="panel-footer">
                           ...
                           <?=(($q['usada'] > 0 ) ? '<i class="fa fa-star fa-2x text-warning"></i> <a href="#" class="btn-lista">Usada</a>'  : '')?>
                            <?php 

                                    foreach ($this->cadastro->prova_questoes_us($q['codigo']) as $key => $value) {
                                     ///($this->cadastro->prova_questoes_detalhe($value->CD_PROVA));
                                    foreach ($this->cadastro->prova_questoes_detalhe($value->CD_PROVA) as $key => $value) {
                                        echo  "<div class='lista'><br><strong>Prova de número</strong> : ".$value->NUM_PROVA." <strong><br> Título:</strong> ".$value->TITULO ."<br></div>";
