    ...
    <div class="modal-content">
            ...
            <table class="table table-bordered table-hover">
                ...
                <? } ?>
                 >
                    ...
                    <td class="text-center"><?='x'.$l['VL_QUESTAO']?></td>
                    <td class="bg-warning text-center"><?=(($l['NR_NOTA'] == '')? '-' : $l['NR_NOTA'])?></td>
                    <td class="bg-warning text-center"><?=(($l['NR_NOTA_DISCURSIVA'] == '')? '-' : $l['NR_NOTA_DISCURSIVA'])?></td>
                    <td class="bg-success text-center"><?=(($l['NOTA'] == '')? '-' : $l['NOTA'])?></td>
                </tr>
                <? } ?>
            </table>
        </div>
        <div class="modal-footer">
            ...
            <button type="button" class="btn btn-primary2" id="frmProvaCartao">
                <i class="fa fa-file-text"></i> Processar Cartões <?php //echo $listar[0]['CD_TIPO_PROVA'].'-'.$original; ?>
            </button>
...
<script>
    $(function() {
        /*AO CLICAR NO BOTÃO PROCESSAR CARTÕES*/
        ...
        $('#<?=$frmId?>').click(function() {
            ...
            function(isConfirm) {
                if (isConfirm) {
                    $.post("<?= base_url(''.$this->session->userdata('SGP_SISTEMA').'/processamento2/'.$frmId.'/'.$listar[0]['CD_TIPO_PROVA'].'-'.$original.'') ?>", {
                    avalProva: <?=$original?>
                    },
                    function(data) {
                        swal("Sucesso!", "Processamento realizado com sucesso!", "success");
