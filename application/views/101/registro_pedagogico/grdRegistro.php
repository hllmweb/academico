    ...
    <tbody>
        <? foreach ($lista as $row) { ?>
            <tr>
                <td>
                    <? if($row['CONT'] > 0){?>
                        <button href="<?=base_url(''.$this->session->userdata('SGP_SISTEMA').'/registro_pedagogico/mdlRegistroAluno/'.$row['CD_ALUNO'].'')?>" data-toggle="frmModalInfo" type="button" id="" class="btn btn-xs btn-primary">
