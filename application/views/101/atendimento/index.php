...
<script type='text/javascript'>
    function txtFiltrar() {
        $("#tblFiltro").html('<?= LOAD ?>');
        $.post("<?= base_url($this->session->userdata('SGP_SISTEMA') . '/atendimento/gridRegistro') ?>", {
    ...
    <div class="row projects">
        <div class="hpanel hgreen">
            <div class="panel-footer">
                <div class="row">
                    <div class="col-xs-2">
                        <button href="<?= base_url('' . $this->session->userdata('SGP_SISTEMA') . '/atendimento/modalRegistro/I') ?>" data-toggle="frmModal" type="button" id="" class="btn btn-labeled btn-primary2">
                        ...
                        <div class="form-group">
                            ...
                            <select name="FTPeriodo" id="FTPeriodo" class="form-control m-b" style="width:150px">
                                <? foreach ($periodo as $per) { ?>
                                    <option value="<?= $per['DC_PERIODO'] ?>" <?= $per['DC_PERIODO'] == $this->session->userdata("SGP_PERIODO") ? "selected" : "" ?>>
                                        <?= $per['DC_PERIODO'] ?>
                                    </option>
                                <? } ?>
                            </select>
