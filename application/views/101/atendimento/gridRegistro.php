    ...
    <tbody>
        <? foreach ($lista as $row) { ?>
            <tr>
                <td>
                    <? if ($row['CONT'] > 0) { ?>
                        <button href="<?= base_url('' . $this->session->userdata('SGP_SISTEMA') . '/atendimento/modalRegistroAluno/' . $row['ID_RESPONSAVEL'] . '-' . $row['CD_ALUNO']) ?>" data-toggle="frmModalInfo" type="button" id="" class="btn btn-xs btn-primary">
                            <?= (($row['CONT'] == 1) ? '( ' . $row['CONT'] . ' ) Atendimento' : '( ' . $row['CONT'] . ' ) Atendimentos') ?> 
                        </button>
                    <? } ?>
                </td>
