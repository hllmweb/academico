...
class Prova_model extends CI_Model {
    ...
    function reprocessar_prova($p){
      $cursor = '';
      // $params = array('name' => ':P_NUM_PROVA',               'value' => $p['num_prova']);
      $params = array(
            array('name' => ':P_NUM_PROVA_PAI',               'value' => $p['num_prova'])
       );

      return $this->db->procedure('BD_SICA', 'SP_REPROCESSA_PROVA_ONLINE', $params);
    }



    //exportação de nota para o rm
    function exporta_nota_rm($p){
       
        $cursor = '';
        $params = array(
            array('name' => ':P_CD_PROVA_PAI',            'value' => $p['cd_prova_pai']),
            array('name' => ':P_CURSOR',                  'value' => $cursor,   'type' => OCI_B_CURSOR)

        );

        return $this->db->procedure('RM', 'SP_EXPORTA_NOTA_RM', $params);
    }
    ...
    function prova_disciplina($p) {
	 ...
	 function prova_disciplina_new($p) {
        $cursor = '';
        $params = array(
            array('name' => ':P_OPERACAO',                'value' => $p['operacao']),
            array('name' => ':P_CD_PROVA',                'value' => $p['prova']),
            ...
            array('name' => ':P_TIPO_QUESTAO',            'value' => $p['tipo']),
            array('name' => ':P_VL_QUESTAO',              'value' => $p['valor']),
            array('name' => ':P_ID_AVAL_PROVA_DISC',      'value' => $p['id_aval_prova_disc']),			
            array('name' => ':P_CURSOR',                  'value' => $cursor, 'type' => OCI_B_CURSOR)
        );
		
        return $this->db->procedure('BD_ACADEMICO','AVAL_MANTER_PROVA_DISC_NEW',$params);
    }
    ...
    function modelVerificaHoraProva($param){

        return $this->db->query("
                                SELECT TITULO FROM BD_SICA.AVAL_PROVA P
                                 WHERE
                                 P.CD_PROVA <> '".$param['P_CD_PROVA']."'
                                 AND
                                 TO_CHAR(P.DT_PROVA,'DD/MM/YYYY') = '".$param['P_DATA']."'
                                 AND
                                 '".$param['P_HR_INICIO']."' BETWEEN  P.HR_INICIO
                                 AND P.HR_FIM AND
                                 P.CD_CURSO = '".$param['P_CD_CURSO']."'
                                 AND
                                 P.ORDEM_SERIE = '".$param['P_ORDEM_SERIE']."'
